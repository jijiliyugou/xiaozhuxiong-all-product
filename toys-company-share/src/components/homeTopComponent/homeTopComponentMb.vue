<template>
  <div class="wrap">
    <div class="myInput" v-if="isShowSearch">
      <el-input
    ...
    <div class="topLauot" v-if="isShowTitle">
      <div class="left">
        <el-image
          @click.native="tohome"
          fit="contain"
    ...
    <div class="menuBox">
      <div class="menu">
        <div
          class="menu_item"
          :class="[nowActive == '/index/home' ? 'active' : '']"
          @click="toLink('/index/home')"
        >
          {{ homeLang.home }}
        </div>
        <div
          class="menu_item"
          :class="[nowActive == '/index/product' ? 'active' : '']"
          @click="toLink('/index/product')"
        >
          {{ homeLang.product }}
        </div>
        <div
          class="menu_item cart"
          :class="[nowActive == '/index/shoppingCart' ? 'active' : '']"
          @click="toLink('/index/shoppingCart')"
        >
          {{ homeLang.ShoppingCart }}
        </div>
        <div
          class="menu_item"
          :class="[nowActive == '/index/myOrder' ? 'active' : '']"
          @click="toLink('/index/myOrder')"
        >
          {{ homeLang.MyOrder }}
        </div>
      </div>
...
export default {
  ...
  watch: {
    ...
    "$route.path": function() {
      this.nowActive = window.localStorage.getItem("nowActive")
        ? window.localStorage.getItem("nowActive")
        : "/index/home";
    }
  },
  computed: {
  ...
  methods: {
    ...
    toLink(value) {
      ...
      window.localStorage.setItem("nowActive", this.nowActive);
      if (value == "/index/product") {
        this.$router.push({
          path: value,
          query: {
            productType: 1
